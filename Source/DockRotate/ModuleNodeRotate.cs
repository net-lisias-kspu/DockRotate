	...
	public class ModuleNodeRotate: ModuleBaseRotate
	{
		...
		protected override AttachNode findMovingNodeInEditor(out Part otherPart)
		{
			...
			if (rotatingNode == null)
				return null;
			Log.detail(this, ".findMovingNodeInEditor(): rotatingNode = {0}", rotatingNode.desc());
			otherPart = rotatingNode.attachedPart;
			Log.detail(this, ".findMovingNodeInEditor(): otherPart = {0}", otherPart.desc());
			if (!otherPart)
				return null;
			Log.detail(this, ".findMovingNodeInEditor(): attachedPart = {0}", rotatingNode.attachedPart.desc());
			return rotatingNode;
		}
		...
		protected override bool setupLocalAxis(StartState state)
		{
			...
			if (rotatingNode == null) {
				Log.detail(this, ".setupLocalAxis({0}): " + "no node \"{1}\"", state, rotatingNodeName);

				List<AttachNode> nodes = part.allAttachNodes();
				for (int i = 0; i < nodes.Count; i++)
					Log.detail(this, ": node[{0}] = {1}", i, nodes[i].desc());
				return false;
			}

			partNodePos = rotatingNode.position;
			partNodeAxis = rotatingNode.orientation;
			Log.detail(this, ".setupLocalAxis({0}) done: {1}@{2}", state, partNodeAxis, partNodePos);
			return true;
		}
		...
		protected override PartJoint findMovingJoint()
		{
			...
			if (rotatingNode == null || !rotatingNode.owner) {
				Log.trace(this, ".findMovingJoint(): no node");
				return null;
			}

			if (part.FindModuleImplementing<ModuleDockRotate>()) {
				Log.trace(this, ".findMovingJoint(): has DockRotate, NodeRotate disabled");
				return null;
			}
			...
			if (!other) {
				Log.trace(this, ".findMovingJoint({0}): attachedPart is null, try by id = {1}", rotatingNode.id, prevOtherPartFlightID);
				other = findOtherById(prevOtherPartFlightID);
			}
			if (!other) {
				Log.trace(this, ".findMovingJoint({0}): no attachedPart", rotatingNode.id);
				return null;
			}
			if (other.flightID != prevOtherPartFlightID)
				Log.trace(this, ".findMovingJoint({0}): otherFlightID {1} -> {2}", rotatingNode.id, prevOtherPartFlightID,  other.flightID);
			Log.trace(this, ".findMovingJoint({0}): attachedPart is {1}", rotatingNode.id, other.desc());
			other.forcePhysics();
			...
			if (owner.parent == other) {
				...
				Log.detail(this, ".findMovingJoint({0}): child {1}", rotatingNode.id, ret.desc());
			...
			if (other.parent == owner) {
				...
				Log.detail(this, ".findMovingJoint({0}): parent {1}", rotatingNode.id, ret.desc());
				otherPartFlightID = other.flightID;
				return ret;
			}

			Log.trace(this, ".findMovingJoint({0}): nothing", rotatingNode.id);
			return null;
		}
